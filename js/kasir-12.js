...
function tambahKeKeranjang(barangId, namaBarang, hargaSatuan, hargaButir) {
  ...
  // Tentukan harga yang digunakan (hargaSatuan atau hargaButir)
...
function hapusBarangDariKeranjang(barangId) {
  ...
  // Hapus elemen barang di UI
  const barangElement = document.getElementById(`item-${barangId}`);
  if (barangElement) {
    barangElement.remove();
  }
...
function updateJumlah(barangId, hargaAktual, jumlah) {
  // Ambil data keranjang dari localStorage
  let keranjang = JSON.parse(localStorage.getItem("keranjang")) || [];
  ...
  if (existingItem && jumlah > 0) {
    existingItem.jumlahKeluar = parseInt(jumlah);
    localStorage.setItem("keranjang", JSON.stringify(keranjang));
...
// Fungsi untuk menangani saat input kehilangan fokus (onblur)
function handleBlur(barangId, hargaAktual, inputElement) {
  const jumlah = inputElement.value;
  updateJumlah(barangId, hargaAktual, jumlah);
}

// Fungsi untuk memuat data barang dari localStorage saat halaman dimuat
window.onload = function () {
  // Ambil data keranjang dari localStorage
  const keranjang = JSON.parse(localStorage.getItem("keranjang")) || [];

  // Tampilkan kembali barang yang ada di keranjang
